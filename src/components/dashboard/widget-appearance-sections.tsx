...
// Basic/Advanced tab content for the Wall of Love creation flow's step 2.
// Same WidgetAppearanceState fields WidgetAppearanceForm renders for the
// standalone editor, split into two purpose-built groups for tabs — kept as
// separate components (some duplication vs. WidgetAppearanceForm) rather
// than refactoring that component, so the standalone editor's existing,
// working layout stays completely untouched. Wall-only, since this tabbed
// UI is only used by the Wall of Love flow (Single testimonial reuses the
// flat WidgetAppearanceForm as-is).
//
// Layout itself is never shown here — it's chosen in step 1 and is not
// re-asked, only the fields *that layout* needs (columns, autoplay, etc.).
...
export function WidgetAdvancedFields({
  state,
  onChange,
}: {
  state: WidgetAppearanceState;
  ...
  return (
    <div className="space-y-4">
      {(state.layout === "masonry" ||
        (state.layout === "masonry-animated" && state.scrollDirection === "vertical")) && (
        <div className="space-y-2">
          <Label>Columns</Label>
          <Select
        ...
        <>
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(v) =>
                onChange({ scrollDirection: v as WidgetAppearanceState["scrollDirection"] })
              }
            >
              <SelectTrigger>
                <SelectValue>
          ...
          {state.scrollDirection === "horizontal" && (
            <div className="space-y-2">
              <Label>Rows</Label>
              <Select value={String(state.rows)} onValueChange={(v) => onChange({ rows: Number(v) })}>
                <SelectTrigger>
          ...
          <p className="text-sm text-muted-foreground">
            Pauses automatically while the visitor hovers, and never grows past the height below
            no matter how many testimonials you have.
          </p>
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) =>
                onChange({ maxHeight: e.target.value === "" ? "" : Number(e.target.value) })
              }
            />
          </div>
        </>
        ...
        <>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Keeps the widget compact with an internal scroll instead of growing the host page
                as testimonials pile up.
              </p>
            </div>
            <Switch
